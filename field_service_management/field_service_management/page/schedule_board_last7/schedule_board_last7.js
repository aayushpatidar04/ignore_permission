frappe.pages['schedule-board-last7'].on_page_load = function(wrapper) {
	...
	$(document).ready(function () {
		...
		$(document).on("click", ".submit", function () {
			...
			frappe.call({
				method: "field_service_management.field_service_management.page.schedule_board_last7.schedule_board_last7.save_form_data",
				args: {
					form_data: formData
				},
		...
		$(document).on('shown.bs.modal', '#mapModal', function () {
			...
			// Initialize the map
			liveMap = L.map(mapContainerId).setView([20.0, 78.0], 5); // Initial view centered on India
			L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
				maxZoom: 19,
				attribution: '&copy; OpenStreetMap contributors'
			}).addTo(liveMap);
